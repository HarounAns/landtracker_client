...
const getInactiveFeed = async (key: any): Promise<GetUserFeedApiResponse> => {
    const url = (
        Boolean(key.length)
            ?
            `${LANDTRACKER_API_URL}/users/${USERNAME}/inactive_feed?nextKey=${key}`
            :
            `${LANDTRACKER_API_URL}/users/${USERNAME}/inactive_feed`
    );
...
const InactiveFeed: React.FunctionComponent = () => {
    ...
    useEffect(() => {
        const loadInitialPage = async () => {
            const { data: { feed, nextKey } }: GetUserFeedApiResponse = await getInactiveFeed('');
    ...
    useEffect(() => {
        const handleScroll = async () => {
            ...
            const { data: { feed, nextKey } }: GetUserFeedApiResponse = await getInactiveFeed(encodeURIComponent(JSON.stringify(nextPageKey)));
            const uniqueFeed: ZillowItem[] = [];
            const zpids: string[] = [];
